...
- **Phase 4C (complete):** Authentication, authorization & verification — user registration/login/JWT (`/api/auth`), database-backed institution memberships (`owner`, `representative`, `faculty`, `student`), platform reviewer role (`users.is_platform_reviewer`) that can verify any institution platform-wide without membership, automatic owner membership on creation, protected institution PATCH, verification state machine (`unverified` → `pending_review` → `verified`/`rejected`/`suspended`), server-controlled audit fields (`verified_by`, `verified_at`, `verification_note`), frontend auth (AuthContext, Login/Register, ProtectedRoute, UserMenu, JWT auto-attach, 401/403 handling), ownership-aware edit UI.
